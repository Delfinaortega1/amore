...
// Se agrega un botón flotante para iniciar la conexión serial
// (el navegador requiere un gesto del usuario para abrir el puerto)
...
`;
btnSerial.addEventListener('mouseenter', () => btnSerial.style.background = 'rgba(155,93,229,0.4)');
btnSerial.addEventListener('mouseleave', () => btnSerial.style.background = 'rgba(155,93,229,0.2)');
...
btnSerial.addEventListener('click', async () => {
  try {
    ...
    btnSerial.style.borderColor = 'rgba(0,245,212,0.4)';
    btnSerial.style.pointerEvents = 'none';

    leerSerial();
  } catch(e) {
    console.warn('Serial no disponible:', e);
    btnSerial.textContent = '✗ Error al conectar';
    btnSerial.style.color = 'rgba(255,100,100,0.8)';
  }
});
  ...
  try {
    while (true) {
      ...
      if (value) {
        ...
        bufferSerial = lineas.pop(); // guardar lo incompleto
        lineas.forEach(linea => procesarMensaje(linea.trim()));
      }
    }
  } catch(e) {
    console.warn('Error leyendo serial:', e);
  }
}
...
function irAEstado2(quienQueda) {
  ...
  setTimeout(() => {
    ...
    if (quienQueda === 'A') {
      personaB = false;
      huellaB.classList.remove('activa');
      p1.classList.add('petalo-A');
      p2.classList.add('petalo-A');
      p5.classList.add('titilar');
      setEstado('Falta una segunda persona', 'activo');
    } else {
      personaA = false;
      huellaA.classList.remove('activa');
      p4.classList.add('petalo-B');
      p5.classList.add('petalo-B');
...
function activarPersonaA() {
  if (personaA) return;
  if (conexionHecha) return;

  personaA = true;
  huellaA.classList.add('activa');

  limpiarClasesPetalo(p1);
  limpiarClasesPetalo(p2);
  p1.classList.add('petalo-A');
  p2.classList.add('petalo-A');

  limpiarClasesPetalo(p5);
  p5.classList.add('titilar');
...
function activarPersonaB() {
  ...
  limpiarClasesPetalo(p4);
  limpiarClasesPetalo(p5);
  p4.classList.add('petalo-B');
  p5.classList.add('petalo-B');

  limpiarClasesPetalo(p3);
  p3.classList.add('titilar-b');

  centroGlow.setAttribute('opacity', '0.6');
  centroGlow.setAttribute('r', '10');
...
function pasarAEsperandoToque() {
  limpiarClasesPetalo(p3);
  p3.classList.add('titilar');
  sensor.classList.add('visible');
  setEstado('Toquen la planta', 'activo');
  centroGlow.setAttribute('opacity', '0.8');
  centroGlow.setAttribute('r', '12');
}

// ——— ESTADO 3B: CONEXIÓN TOTAL ———
...
function activarConexion() {
  ...
  [p1,p2,p3,p4,p5].forEach(p => {
    limpiarClasesPetalo(p);
    p.classList.add('conexion');
  });

  centroGlow.setAttribute('r', '18');
  centroGlow.setAttribute('opacity', '1');
  ...
  setTimeout(() => {
    setEstado('CONEXIÓN COMPLETA', 'conexion-total');
  }, 800);

  setTimeout(() => abrirProyeccion(), 2000);

  // ——— ESTADO 4: FINAL después de 20 segundos ———
...
// ——— TECLADO (respaldo para pruebas sin Arduino) ———
...
function reproducirSonidoDesconexion() {
  try {
    ...
    gain.gain.setValueAtTime(0.5,  ctx.currentTime + 1.6);
